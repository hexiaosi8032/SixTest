...
- (void)viewDidLoad {
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"聊天框-自己"] stretchableImageWithLeftCapWidth:30 topCapHeight:20]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"聊天框-别人"] stretchableImageWithLeftCapWidth:30 topCapHeight:20]];
...
- (void)sendTextMessage:(NSString *)text
{
    ...
    ext = @{
            ...
            @"ID" : [User sharedInstance].userID,
            @"nickName" : [User sharedInstance].nickName,
            @"userName" : [User sharedInstance].userName,
            @"userRole" : [User sharedInstance].userRole
            };
    
    NSString *dicStr = [[NSString alloc] initWithData:[NSJSONSerialization dataWithJSONObject:ext options:nil error:nil] encoding:NSUTF8StringEncoding];
    [self sendTextMessage:text withExt:@{
                                         @"user" : dicStr
                                         }];
//    NSDictionary *ext = nil;
//    if (self.conversation.type == EMConversationTypeGroupChat) {
