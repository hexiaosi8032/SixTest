...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    //修改
    ChatExtModel *chatModel = nil;
    NSString *userStr = model.message.ext[@"user"];
    if (userStr) {
        NSDictionary *userDic = [NSJSONSerialization JSONObjectWithData:[userStr dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingAllowFragments error:nil];
        chatModel = [ChatExtModel mj_objectWithKeyValues:userDic];
    }
    if (userStr) {
        self.avatarView.layer.borderWidth = 0.5;
        self.avatarView.layer.borderColor = [UIColor redColor].CGColor;
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:chatModel.headPortrait] placeholderImage:[UIImage imageNamed:@"默认头像"]];
    }else{
        self.avatarView.image = [UIImage imageNamed:@"默认头像"];
    }
    
    if ([chatModel.userRole isEqualToString:@"EXPERT"]) {
        _nameLabel.text = [NSString stringWithFormat:@"专家: %@",chatModel.nickName];
    }else{
        _nameLabel.text = chatModel.nickName;
    }
